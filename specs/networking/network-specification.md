...
Noise handshakes will be used for mainnet. libp2p Noise support [is in the process of being standardised](https://github.com/libp2p/specs/issues/195) in the libp2p project.

Noise will presumably include IX, IK and XX handshake patterns, and may rely on Curve25519 keys, ChaCha20 and Poly1305 ciphers, and SHA256 as a hash function. These aspects are being actively debated in the referenced issue [ETH 2.0 implementers are welcome to comment and contribute to the discussion.]
...
- `REQ_MAX_SIZE`: The max size of uncompressed RPC messages that clients will allow. 
 Value: TBD
...
Topic strings have form: `/TopicName/TopicEncoding`. This defines both the type of data being sent on the topic and how the data field of the message is encoded. (Further details can be found in [Messages](#Messages)).
...
For the interoperability testnet, all peers will support all capabilities defined in this document (gossip layer, full RPC suite, discovery protocol), therefore the ENR record does not need to carry capability information, as it would be superfluous.

#### Mainnet

On mainnet, ENRs MUST include a structure enumerating the capabilities offered by the peer in an efficient manner. Proposals include using namespaced bloom filters mapping capabilities to specific protocol IDs supported under that capability, but this is TBD and nonessential for the interoperability testnet.
...
Each RPC message type is segregated into its own libp2p protocol ID, which is a case-sensitive UTF-8 string of the form:
...
We use ONE stream PER request/response interaction. Streams are closed when the RPC finishes, whether in success or in error.
...
The encoding-dependent header may carry metadata or assertions such as the encoded payload length, for integrity and attack proofing purposes. It is not strictly necessary to length-prefix payloads, because RPC streams are single-use, and stream closures implicitly delimit the boundaries, but certain encodings like SSZ do, for added security.

`encoded-payload` has a maximum byte size of `RPC_MAX_SIZE`. 

Clients MUST ensure the payload size is less than or equal to `RPC_MAX_SIZE`, if not, they SHOULD reset the stream immediately. Clients tracking peer reputation MAY decrement the score of the misbehaving peer under this circumstance.
...
If any of these timeouts fire, the requester SHOULD reset the stream and deem the RPC operation to have failed.
...
The token of the negotiated protocol ID specifies the type of encoding to be used for the RPC interaction. Two values are possible at this time:
...
### Req/Resp Message Specification

*Note: The following is mostly borrowed from the[ RPC Interface](https://github.com/ethereum/eth2.0-specs/blob/192442be51a8a6907d6401dffbf5c73cb220b760/specs/networking/rpc-interface.md) specification with libp2p-specific modifications.*
...
Clients MUST support requesting blocks since the start of the weak subjectivity period.
...
**Why are we using TCP/QUIC/Websockets?**
...
multiselect 2.0 is currently being conceptualised. Debate started [on this issue](v), but it got overloaded – as it tends to happen with large conceptual OSS discussions that touch the heart and core of a system.
...
-  Gossip layer: since gossipsub has a framing protocol and exposes an API, we compress the payload (when dictated by the encoding token in the topic name) prior to publishing the message via the API. No length prefixing is necessary because protobuf takes care of bounding the field in the serialised form.
-  Req/Resp layer: since we define custom protocols that operate on byte streams, implementers are encouraged to encapsulate the encoding and compression logic behind MessageReader and MessageWriter components/strategies that can be layered on top of the raw byte streams.
